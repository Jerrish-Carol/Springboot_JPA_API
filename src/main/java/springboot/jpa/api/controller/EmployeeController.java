...
@RestController  														//@Controller + @ResponseBody
																		//@Controller  
																		/* controller annotation - server will send all the HTTP request to these classes */
public class EmployeeController {
	...
	private EmployeeService eService;
		...
		@GetMapping("/employees") 									   // if we use RequestMethod.GET/PUT/POST we can use this annotation instead of @requestMapping
		
																		//@RequestMapping(value= "/employees", method= RequestMethod.GET)  // in order to map these handler methods to postman 
																		                                      //parameters are URI and HttpMethod
																		
																		//@ResponseBody // returns the response i.e here it returns "displaying list of employees" 
																		
																		//ResponseEntity is used to return not only data but also status code and other details
	...
	public ResponseEntity<Employee> getEmployee(@PathVariable /*("id")*/ Long ID)   { //pathvariable is the variable that is given in URL 
		if(eService.getSingleEmployee(ID) != null) {															// Long - datatype id - variable								// if /employees/{id} - id is the same as in @PathVariable("id") then change it to -> public String getEmployee(@PathVariable Long id)				
	...
	@DeleteMapping("/employees")
																	 ...
																	 * public void deleteEmployee(@RequestParam//("id")// Long id) {
																	 // if Long id - id is the same as in @RequestParam("id") then change it to -> @RequestParamLong id
	...
	public ResponseEntity<Employee> saveEmployee (@Valid @RequestBody Employee employee)  { //@RequestBody works with get,put,post so for 
	    	return new ResponseEntity<>(eService.saveEmployee(employee), HttpStatus.CREATED);
	    }

	
	@PutMapping("/employees/{ID}")
	public ResponseEntity<Employee> updateEmployee ( @PathVariable Long ID, @Valid @RequestBody Employee employee) { //@RequestBody works with get,put,post so for 
		
		if(eService.getSingleEmployee(ID).getIsAccountLocked() == "no") {
